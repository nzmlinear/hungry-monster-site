...
const warningNull = document.getElementById('warningnull');

searchBtn.addEventListener('click', function () {
    const keyword = document.getElementById('keyword').value;
    data_container.innerHTML = '';
    if (keyword === '') {
        warningNull.style.display = 'block';
    } else {
        getFood(keyword);
...
const renderFoodInfo = food => {
    ...
    <h4>${food.strMeal}</h4>
    <h5 class="pt-3 pb-2">🔥</i> Ingredients</h5>
    <ul class="list-unstyled mb-0">
        <li>✅</i>${food.strMeasure1}, ${food.strIngredient1}</li>
        <li>✅</i>${food.strMeasure2}, ${food.strIngredient2}</li>
        <li>✅</i>${food.strMeasure3}, ${food.strIngredient3}</li>
        <li>✅</i>${food.strMeasure4}, ${food.strIngredient4}</li>
        <li>✅</i>${food.strMeasure5}, ${food.strIngredient5}</li>
        <li>✅</i>${food.strMeasure6}, ${food.strIngredient6}</li>
    </ul>
    ...
    const displayFoods = meals => {
        ...
        if (meals != null) {
            meals.map(food => {
                ...
                warningNull.style.display = 'none';
        }
    };
}
